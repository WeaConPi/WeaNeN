...
export const CalculateResult = (day: number, month: number, hour: number, temp: number, houseTemp: number): INNOutput => {

  const inputValues = new NNInput(day, month,  hour, temp, houseTemp);
...
const optimizeInputValues = ({ day, month, hour, temp, houseTemp }: INNInput): number[] => {
  return [(day + 50) / 100, month / 100, hour / 100, temp / 100, houseTemp / 100];
}
